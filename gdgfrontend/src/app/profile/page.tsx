      ...
      <div className="max-w-md mx-auto">
        <AnimatePresence mode="wait">
          {screen === "profile" ? (
            ...
            <motion.div key={screen} {...pageVariants}>
              <SubPageHeader
                title={
                  screen === "account"
                    ? "Bio-data"
                  ...
                  <div className="text-center space-y-6 pt-10">
                    ...
                    <div>
                      ...
                      <p className="text-secondary-text text-sm mt-2 max-w-xs mx-auto">
                        A prototype platform for reporting and recovering lost
                        items on campus. Built For GDG TechFest
                      </p>
                    </div>
    ...
    <div className="flex flex-col items-center text-center">
      ...
      >
        {photoUrl ? (
          ...
          <UserIcon
            size={32}
            className={light ? "text-white" : "text-secondary-text"}
          />
        )}
      </div>
      <h2
      ...
      <p
        className={`text-sm ${light ? "text-blue-100" : "text-secondary-text"}`}
      >
        {email}
      </p>
    </div>
...
function SubPageHeader({ title, onBack }: SubPageHeaderProps) {
  return (
    <div className="flex items-center gap-4 py-2">
      <button
        onClick={onBack}
        className="flex h-10 w-10 items-center justify-center rounded-full bg-btn-bg text-secondary-text hover:bg-btn-hover hover:text-foreground transition-colors"
      >
        <ChevronLeft size={20} />
      </button>
...
function SwitchCard({ title, subtitle, checked, onToggle }: SwitchCardProps) {
  return (
    ...
    >
      ...
      <div
        className={`h-6 w-11 rounded-full p-1 transition-colors ${checked ? "bg-accent-blue" : "bg-secondary-text/30"}`}
      >
        <div
